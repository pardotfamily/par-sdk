...
- **Fees to holders.** A launch may name `addresses.holderVault` as its creator fee recipient (`feesToHolders: true` on the indexer row, `?feesToHolders=1` filters). Its creator share is paid to holders pro rata every hour, in the asset it came in (ETH or the pool's quote from buys, the token from sells), through `addresses.disperse` (`PairPadDisperseV2`: one call per asset, tagged with the launch, one `Paid` event per recipient). `indexer.distributions(token)` lists the payouts with per-asset `totals`, `indexer.rewards(owner, token)` what a wallet got, also per asset.
- **Buyback & burn.** A launch may instead name `addresses.burnVault` as its creator fee recipient (`feesBurned: true`, `feeMode: "burn"`, `?feeMode=burn` filters). Its creator share buys the token back in its own pool every hour and is burned together with the share paid in the token. `indexer.burns(token)` lists the rounds; the row's `feeBurnedToken` is the running total. On ordinary launches the creator can also burn the fee tokens it claimed from the token page; `creatorBurnedToken` is the running total of that.
- **Price floor.** A launch may name `addresses.floorVault` as its creator fee recipient (`feesFloor: true`, `feeMode: "floor"`, `?feeMode=floor` filters). Every creator fee in the quote becomes one locked buy wall in the launch pool, placed at the highest price at which the wall could absorb the entire circulating supply; the token side and whatever the wall bought are burned. The wall is re-placed after every collection (`WallMoved`) and the floor only rises. The row's `floor` array has the current wall per market (`floorPriceEth`, `floorMarketCapEth`, `quoteInWall`, `circulating`); `indexer.floor(token)` adds the history. `feeMode` is `creator`, `holders`, `burn` or `floor` on every row.
